...
6. **`Inventory_StockOpnames`** (header) + **`Inventory_StockOpnameLines`** — physical count reconciliation. Header: `OpnameNumber` (generated via `DocumentNumberGeneratorService.GetNextNumberAsync("StockOpname")`), FK→Warehouse, `OpnameDate`, `Status` (`Draft`/`Submitted`/`Approved`). Lines: FK→Header, FK→Item, FK→Batch (nullable), `SystemQuantity` (snapshot at count time), `CountedQuantity`, `VarianceQuantity` (computed), `Notes`.
...
2. **Warehouse master** — reuse existing `Setting_Warehouses` as-is, or does it need new fields (capacity, warehouse type: Main/Quarantine/Vendor) to support the quarantine/subcontractor-transfer flows above? Leaning: add `WarehouseType` (`Main`/`Quarantine`/`Vendor`) column to `Setting_Warehouses` since it's still a Settings-owned static reference, not new business logic.
...
- Landed cost allocation formula (how allocated amounts spread across batch quantity) not yet decided — needs a follow-up design pass once Finance's costing approach is confirmed.
