...
1. ✅ **Done**: `LifecycleStatus` column added to `Procurement_PurchaseOrders`/`Procurement_PurchaseRequisitions` ([0012_Procurement_LifecycleStatus.sql](../../src/Sugentra.ERP.Migrator/Scripts/Procurement/0012_Procurement_LifecycleStatus.sql), backfilled from existing `Status` values), plus a `Procurement_StatusTransitions`-backed `IStatusTransitionRepository.IsAllowedAsync(categoryGroup, from, to)` used by `PurchaseOrderUseCase.ApplyReceiptAsync` before moving `LifecycleStatus`. `FinalizeApprovedAsync` on both PR and PO sets `LifecycleStatus = "Open"` the moment approval completes. `Procurement_StatusTransitions`' seed data was corrected in [0013_Procurement_StatusTransitions_Fix.sql](../../src/Sugentra.ERP.Migrator/Scripts/Procurement/0013_Procurement_StatusTransitions_Fix.sql) (dropped bogus `Draft`/`PendingApproval` rows, renamed `Approved` → `Open` as the lifecycle baseline). Still not surfaced anywhere in the UI, and `ApplyReceiptAsync` itself is still not called by anything (next step).
2. ✅ **Done**: Goods Receipt ↔ PO link. `Inventory_GoodsReceipts.PurchaseOrderId` (nullable FK, [0030_Inventory_GoodsReceipts_PurchaseOrderId.sql](../../src/Sugentra.ERP.Migrator/Scripts/Inventory/0030_Inventory_GoodsReceipts_PurchaseOrderId.sql)) — when set, `GoodsReceiptUseCase.FinalizePostAsync` groups the receipt's lines by `ItemId` and calls `IPurchaseOrderReceiptService.ApplyReceiptAsync`. The contract was changed from `PurchaseOrderLineUpdate(PurchaseOrderLineId, qty)` to `PurchaseOrderReceiptItemUpdate(ItemId, qty)` since Inventory has no knowledge of Procurement's internal PO line ids (module isolation) — `PurchaseOrderUseCase.ApplyReceiptAsync` now resolves ItemId → its own PO line(s) and distributes the received qty across them up to each line's remaining quantity. **Not yet wired in the UI** — `CreateGoodsReceiptRequest`/`UpdateGoodsReceiptRequest` accept `PurchaseOrderId` at the API level, but the UI's GoodsReceipts Create/Edit forms have no dropdown for it yet (fast follow-up, not started).
3. ✅ **Done**: PR Cancel/Close. New `Procurement_PurchaseRequisitionLines.ClosedQuantity` column ([0014_Procurement_PurchaseRequisitionLines_ClosedQuantity.sql](../../src/Sugentra.ERP.Migrator/Scripts/Procurement/0014_Procurement_PurchaseRequisitionLines_ClosedQuantity.sql)), new permission `PurchaseRequisition_Cancel` ([0015_Seed_PurchaseRequisition_Cancel_Permission.sql](../../src/Sugentra.ERP.Migrator/Scripts/Procurement/0015_Seed_PurchaseRequisition_Cancel_Permission.sql)). `PurchaseRequisitionUseCase.CloseRemainderAsync` force-closes each line's `Quantity − OrderedQuantity − ClosedQuantity` remainder (validated via `IStatusTransitionRepository`), sets `LifecycleStatus = "Closed"`, logs the reason via `IAuditLogService`. `OrderStatus` in responses now folds `ClosedQuantity` in (reports `"Closed"` when nothing was ordered and the remainder got closed, otherwise `FullyOrdered` once `Ordered + Closed >= Requested`). UI: "Close Remainder" button + reason modal on PR Detail page (visible when `Status == "Approved" && LifecycleStatus == "Open"` and the user has the new permission).
