...
[Route("api/master-data/bill-of-materials")]
[Authorize]
public class BillOfMaterialsController(BillOfMaterialUseCase useCase, BillOfMaterialListQuery listQuery) : ApiControllerBase
{
    [HttpGet]
    ...
    [HttpGet("paged")]
    [Authorize(Policy = "MasterData_View")]
    public async Task<IActionResult> GetPaged([FromQuery] BillOfMaterialListRequest request) => Success(await listQuery.GetPagedAsync(request));
    ...
    public async Task<IActionResult> GetById(long id)
    {
        ...
        return response is null ? Failure("Bill of materials not found.", StatusCodes.Status404NotFound) : Success(response);
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateBillOfMaterialRequest request)
    {
        ...
        return SuccessCreated($"api/master-data/bill-of-materials/{created.Id}", created);
    }

    [HttpPut("{id:long}")]
    [Authorize(Policy = "MasterData_Edit")]
    public async Task<IActionResult> Update(long id, [FromBody] UpdateBillOfMaterialRequest request)
    {
        var result = await useCase.UpdateAsync(id, request);
        return result.IsSuccess
            ? Success(result.Value, "Bill of materials updated successfully.")
    ...
    public async Task<IActionResult> Delete(long id)
    {
        ...
        return result.IsSuccess
            ? SuccessMessage("Bill of materials deleted successfully.")
            : Failure(result.Error!, StatusCodes.Status404NotFound);
    }
}
